...
function helloScreen(next:HelloResult) {
    ...
    return (
        $.scene({ init : () => { tb.readInput((err, value) => { if (!err && value) next(value); }) }})(
...
function fullScreenWithCustomCamera(cam:Camera, player:string, started:() => void, result:(msg:string) => void) {
    return  (
        $.scene({ init: started, event : (msg) => { result(msg); } })(
...
const gameOverText = fs.readFileSync(path.resolve(__dirname, '..', '..', '..', 'resources', 'gameOver.txt'), 'utf-8');
